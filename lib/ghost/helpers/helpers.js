...
module.exports = function getHelpers(config) {
    ...
    coreHelpers.meta_title = function(options) {
        ...
        if (_.isString(this.relativeUrl)) {
            ...
            // Homepage title
            if (!this.relativeUrl || this.relativeUrl === '/' || this.relativeUrl === '/index.html' || this.relativeUrl === '') {
                title = blog.title;
            // Author title
            } else if (this.author && !this.post) {
            ...
            // Post title
                ...
                title = this.post.title + `${this.author ? `- ${this.author.name}` : ''}`;
            } else {
    ...
    // Add keywords
    // https://github.com/TryGhost/Ghost/blob/1.4.0/core/server/data/meta/keywords.js
    coreHelpers.meta_keywords = function(options) {
      let keywords, blog
      console.log(colors.yellow('this.post', util.inspect(this.post, { depth: null })))
    }
        ...
        if (_.isString(this.relativeUrl)) {
          ...
          if (!this.relativeUrl || this.relativeUrl === '/' || this.relativeUrl === '/index.html' || this.relativeUrl === '') {
            ...
            // Author page desc
                ...
                description = this.tag.name + ' - ' + blog.title;
            // Post page desc
            } else if (this.post || /\/page\//.test(this.relativeUrl)) {
              var tagList = this.post.tags.map(t => {
                return t.name
              }).join(',')

              var _desc, desc
              /*jslint regexp:true */
              _desc = String(this.html).replace(/<\/?[^>]+>/gi, '')
                         .replace(/(\r\n|\n|\r)+/gm, ' ');
              /*jslint regexp:false */

              desc = new hbs.handlebars.SafeString(
                downsize(_desc, { words: 100 })
              )
              description = desc + tagList
            }
        }
